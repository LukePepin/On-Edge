...
class HanoiSolver(Node):
    ...
    def define_positions(self):
        ...
        self.d3_pickup_t2_p1_tcp = (0.28778, 0.38134, 0.34510)
    ...
    def move_disk(self, disk, from_tower, from_level, to_tower):
        ...
        if not self.move_joints(from_front_joints, speed=0.2):
            return False
        time.sleep(0.5)
        
        self.get_logger().info(f'2. Match Z height: {front_z:.4f}m → {pickup_z:.4f}m')
        if not self.move_to_xyz(front_x, front_y, pickup_z, speed=0.15):
            return False
        time.sleep(0.5)
        ...
        if not self.move_to_xyz(pickup_x, pickup_y, dropoff_z, speed=0.1):
            return False
        time.sleep(0.2)
        ...
        if not self.move_to_xyz(dropoff_x, dropoff_y - 0.040, dropoff_z - 0.050, speed=0.20):
            return False
        time.sleep(0.5)
        
        self.get_logger().info(f'10. Retreat to T{to_tower}F')
        if not self.move_joints(to_front_joints, speed=0.2):
            return False
        time.sleep(0.5)
