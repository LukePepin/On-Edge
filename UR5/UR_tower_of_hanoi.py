#!/usr/bin/env python3
"""
Tower of Hanoi Solver - Pure Cartesian Linear Motion
Uses hardcoded TCP positions to avoid arcing
"""
...
class HanoiSolver(Node):
    ...
    def define_positions(self):
        ...
        self.d3_pickup_t2_p1_tcp = (0.28778, 0.38134, 0.33806)
    ...
    def move_disk(self, disk, from_tower, from_level, to_tower):
        ...
        if not self.move_joints(from_front_joints, speed=0.05):
            return False
        time.sleep(0.5)
        
        self.get_logger().info(f'2. Match Z height: {front_z:.4f}m → {pickup_z:.4f}m')
        if not self.move_to_xyz(front_x, front_y, pickup_z, speed=0.10):
            return False
        time.sleep(0.3)
        
        self.get_logger().info(f'3. Move forward to pickup (X,Y only, Z constant)')
        if not self.move_to_xyz(pickup_x, pickup_y, pickup_z, speed=0.10):
            return False
        time.sleep(0.3)
        ...
        if not self.move_to_xyz(pickup_x, pickup_y, dropoff_z, speed=0.10):
            return False
        time.sleep(0.3)
        
        self.get_logger().info(f'6. Move to dropoff X,Y (Z constant at clearance)')
        if not self.move_to_xyz(dropoff_x, dropoff_y, dropoff_z, speed=0.10):
            return False
        time.sleep(0.3)
        
        # === PLACEMENT SEQUENCE ===
        self.get_logger().info(f'7. Lower 50mm')
        if not self.move_to_xyz(dropoff_x, dropoff_y, dropoff_z - 0.050, speed=0.10):
            return False
        time.sleep(0.3)
        ...
        if not self.move_to_xyz(dropoff_x, dropoff_y - 0.040, dropoff_z - 0.050, speed=0.10):
            return False
        time.sleep(0.3)
        
        self.get_logger().info(f'10. Lift back to clearance height')
        if not self.move_to_xyz(dropoff_x, dropoff_y - 0.040, dropoff_z, speed=0.10):
            return False
        time.sleep(0.5)
        
        self.get_logger().info(f'11. Retreat to T{to_tower}F (slow)')
        if not self.move_joints(to_front_joints, speed=0.05):
            return False
        time.sleep(0.5)
    ...
    def solve(self):
        """Execute complete Tower of Hanoi solution"""
        
        self.get_logger().info('\n' + '='*60)
        self.get_logger().info('TOWER OF HANOI SOLVER - FULL SOLUTION')
        ...
        input('\nPress Enter to start Tower of Hanoi solution...')
        ...
        for i, (disk, from_t, from_l, to_t, description) in enumerate(moves, 1):
            ...
            if i < 7:
                input(f'\nMove {i}/7 complete. Press Enter for next move...')
