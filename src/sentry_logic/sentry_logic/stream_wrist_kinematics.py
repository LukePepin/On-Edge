...
class MockPickAndPlaceClient(Node):
    def __init__(self):
        ...
        # Action Client for native UR trajectory execution
        # Bound strictly to passthrough_trajectory_controller to avoid wait_for_server() deadlocks inside __init__
    ...
    def trigger_strike_zone(self, delay_sec):
        ...
        self.get_logger().warn("🚀 TRIGGERING 0.5s KINEMATIC STRIKE!")
        req = Trigger.Request()
    ...
    def run_phase2(self):
        ...
        if not self.attack_fired:
            import threading
            threading.Thread(target=self.trigger_strike_zone, args=(0.5,), daemon=True).start()
